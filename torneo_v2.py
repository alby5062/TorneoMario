...
# --- TITOLO ---
st.title("🏆 Gran Premio di Torino - Circuito Corso Francia")
...
with st.sidebar:
    ...
    # SELEZIONA GIORNATA
    # Ordiniamo le giornate per numero per visualizzarle correttamente
    giornate_unsorted = list(data["giornate"].keys())
    # Logica di ordinamento: estrae il numero dalla stringa "Giornata X"
    ...
    if not giornate_sorted:
        ...
        with st.expander("🗑️ Elimina Giornata", expanded=False):
            ...
            if st.button("Sì, ELIMINA", type="secondary"):
                # 1. Elimina la giornata selezionata
                del data["giornate"][selected_day]

                # 2. Rinumerazione (Opzionale ma consigliata per non avere buchi)
                # Esempio: se ho 1, 2, 3 e cancello la 2 -> la 3 diventa 2.
                new_giornate_dict = {}
                # Riordina ciò che è rimasto
                remaining_keys = sorted(data["giornate"].keys(), key=lambda x: int(x.split(" ")[1]))

                for index, old_key in enumerate(remaining_keys):
                    new_key = f"Giornata {index + 1}"  # Rinomina sequenzialmente partendo da 1
...
with tab1:
    ...
    cols = st.columns(4)
    current_vals = day_data["races"][race_num]  # è una lista di 4 interi [10, 7, 0, 2]

    updated = False
    for i, player in enumerate(players):
        with cols[i]:
            # Troviamo l'etichetta corrente basata sul punteggio salvato
    ...
    if updated:
        save_data(data)  # Salva su file
        # Non facciamo rerun qui per evitare flickering fastidiosi, salva silenziosamente
...
with tab2:
    ...
    with c2:
        st.subheader("🏀 Canestro (Max 15)")
        for i, player in enumerate(players):
            val = st.number_input(f"Basket {player}", min_value=0, max_value=15, value=day_data["basket"][i],
    ...
    with c3:
        st.subheader("🎯 Freccette (Max 20)")
        for i, player in enumerate(players):
            val = st.number_input(f"Darts {player}", min_value=0, max_value=20, value=day_data["darts"][i],
...
with tab4:
    ...
    st.markdown(f"Totale giornate giocate: **{len(giornate_list)}**")

    general_stats = {p: {"MK8": 0, "Skill": 0, "KO": 0, "Totale": 0} for p in players}

    # Aggregazione dati di tutte le giornate

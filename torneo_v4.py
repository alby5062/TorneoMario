...
def load_data():
    if os.path.exists(FILE_DATI):
        with open(FILE_DATI, "r") as f:
            data = json.load(f)
            # MIGRATION FIX: Assicura che i vecchi salvataggi abbiano il campo 'absent'
...
with st.sidebar:
    ...
    if not giornate_sorted:
        ...
        # GESTIONE ASSENZE (Solo Admin)
        if st.session_state.is_admin:
            st.markdown("---")
            st.subheader("🚫 Gestione Assenze")
            st.caption("Spunta chi NON gioca oggi:")

            day_data = data["giornate"][selected_day]
            updated_absent = False
            for i, player in enumerate(players):
                # Checkbox per assenza
                is_absent = st.checkbox(f"{player} Assente", value=day_data["absent"][i], key=f"abs_{selected_day}_{i}")
                if is_absent != day_data["absent"][i]:
                    day_data["absent"][i] = is_absent
                    # Se diventa assente, azzeriamo i punteggi per sicurezza
                    if is_absent:
                        day_data["ko"][i] = 0
                        day_data["basket"][i] = 0
                        day_data["darts"][i] = 0
                        for r in range(12):
                            day_data["races"][f"Gara {r + 1}"][i] = 0
                    updated_absent = True
...
day_data = data["giornate"][selected_day]
absent_flags = day_data["absent"]  # Lista booleana [False, True, False, False]

# --- TABS ---
tab1, tab2, tab3, tab4 = st.tabs(["🏎️ GARE", "🎯 SKILL", "🥇 GIORNATA", "🌍 GENERALE (MEDIA)"])
...
# TAB 3: CLASSIFICA GIORNATA
with tab3:
    d_stats = []
    for i, p in enumerate(players):
        if not absent_flags[i]:  # Mostra solo presenti
...
# TAB 4: CLASSIFICA GENERALE (MEDIA PUNTI)
with tab4:
    ...
    for p, s in gen_stats.items():
        pg = s["Presenze"]
        tot = s["Totale"]
        media = round(tot / pg, 2) if pg > 0 else 0.0

        final_list.append({
            "Giocatore": p,
            "PG": pg,  # Partite Giocate
            "Totale Punti": tot,
            "MEDIA PUNTI": media,
            "Totale KO": s["KO"]
        })

    # Ordina per MEDIA PUNTI
    ...
    if not df_gen.empty:
        ...
        st.markdown(f"### 👑 Leader: <span style='color:#e0bc00'>{leader}</span>", unsafe_allow_html=True)
        st.caption(
            "*La classifica è ordinata in base alla Media Punti per garantire equità anche con numero di partite diverso.*")